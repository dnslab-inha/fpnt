...
extern "C" void P_fast_bfm_fill(std::string& option, nlohmann::json& record, const std::string& granularity, const std::string& key, const std::string &field) {
    std::string original = record[field].get<std::string>();
...
extern "C" void P_bfm_fill(std::string& option, nlohmann::json& record, const std::string& granularity, const std::string& key, const std::string &field) {
    ...
    // target_param_key: the key to find in the string 'cnt' (e.g.,: φ11); convert alphabet (phi -> φ, psi -> ψ)
    ...
    // cnt can be split using delimiter ";" and each line consists of one "scidx," and several "key:value".
    // example: ...;-122, φ11:41, φ21:34, ψ21:6, ...;...
