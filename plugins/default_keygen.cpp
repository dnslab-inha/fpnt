 ...
 * udp.srcport, udp.dstport, _ws.col.def_src, _ws.col.def_dst); suitable for encrypted traffic
 * analysis such as TLS or QUIC; note that the host with smaller port number is appeared first (to
 * identify well-known service port quickly) and if the port numbers are the same, the host with
 * smaller IP address is appeared first.
 *
 */
...
/** @brief A default key generator for "directional" Flow records assuming only TCP/IP, UDP/IP
 * datagrams are available for efficient generation; use 4 tuple fields (tcp.srcport, tcp.dstport,
 * udp.srcport, udp.dstport _ws.col.def_src, _ws.col.def_dst); suitable for encrypted traffic
 * analysis such as TLS or QUIC; key string is "srcIP:srcPort,dstIP,dstPort".
 *
 */
