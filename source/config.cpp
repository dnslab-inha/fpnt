...
namespace fpnt
{
    nlohmann::json parse(int argc, char* argv[]) {
        ...
        app.add_option("-t,--output-type", outputType, "Output file type (Currently only CSV is supported)");
        ...
        if (inputPcapPath != "")
            config["input_pcap_path"] = inputPcapPath;
        ...
        if (pluginsPath != "")
            config["plugins_path"] = pluginsPath;

        // TODO: CSV output_type override

        if(std::filesystem::absolute(config["input_pcap_path"].get<std::string>()) == std::filesystem::current_path()) {
            std::cerr << "input_pcap_path must not be the current working directory!" << std::endl;
            exit(1);
        }

        if(std::filesystem::absolute(config["output_path"].get<std::string>()) == std::filesystem::current_path()) {
            std::cerr << "output_path must not be the current working directory!" << std::endl;
            exit(1);
        }

        if (config["input_pcap_path"].get<std::string>() == "" || config["output_path"].get<std::string>() == "" || config["output_type"].get<std::string>() == "") {
            std::cerr << "config.json does not have the default parameter! check input_pcap_path, output_path, and output_type!" << std::endl;
            exit(1);
        }
