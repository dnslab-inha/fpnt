...
int run_fpnt(const std::string& config, const std::string& input_dir, const std::string& output_dir) {

    std::string executable = std::string(FPNT_BINARY_DIR) + "/_deps/fpnt-build/standalone/fpnt";
    std::string plugin_path = std::string(FPNT_BINARY_DIR) + "/_deps/fpnt-build/plugins/libFPNT_PLUGINS.so";
    
    // Use cd to project source directory so relative paths in config files work properly
    std::string cmd = "cd " + std::string(FPNT_SOURCE_DIR) + " && " + 
                      executable + 
                      " -c " + config + 
                      " -i " + input_dir + 
                      " -o " + output_dir + 
